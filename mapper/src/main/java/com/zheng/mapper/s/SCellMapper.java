...
import com.zheng.pojo.s.SCell;
import com.zheng.pojo.s.SCellEx;
...
public interface SCellMapper extends BaseMapper<SCell> {
    @Select("select * from `d_file` df\n" +
            ...
            "where df.`CHECK_TAG`='1' and df.DESIGN_CELL_TAG='1' and sc.check_tag=#{tag} or sc.check_tag=#{tag2}")
...
/*    @Select("select * from `d_file` df\n" +
   ...
   */
    @Select("select * from `d_file` df\n" +
            "left join s_cell sc on df.PRODUCT_ID = sc.PRODUCT_ID\n" +
            "where df.`CHECK_TAG`='1'  and sc.check_tag=#{tag1} or sc.check_tag=#{tag12}\n"+
            "and df.DESIGN_CELL_TAG='1'")
